...
def model():
    ...
    SRCNN.compile(optimizer = LRMultiplier(Adam(lr=0.00001), {'layer_1':1, 'layer_2':1, 'layer_3':0.1})
...
def train():
    ...
    srcnn_model.load_weights('checkpoint/SRCNN_checkpoint_scale4.h5')
    print(srcnn_model.summary())
    
    '''
    data, label = ps.read_training_data('training_sample/train_91_scale4.h5')
    val_data, val_label = ps.read_training_data('training_sample/cross_val_Set5_scale4.h5')
    '''
    ...
    checkpoint_filepath = 'checkpoint/SRCNN_checkpoint_scale4.h5'
    ...
    for i in range(0,1):
        '''
        srcnn_model.fit(x = data, y = label, batch_size = 128, epochs = 20, verbose = 1,
                       callbacks = checkpoint_callbacks, validation_data = (val_data, val_label), shuffle = True)

        #srcnn_model.save('model_weight/SRCNN_check_91_scale4_same_2_1.h5')
        '''
