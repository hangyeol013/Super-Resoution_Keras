...
def EDSR(scale, input_image, n_feats, n_resblocks):
    ...
    x = layers.Conv2D(filters = n_feats * (scale ** 2), kernel_size = (3,3), padding = 'same',
                     bias_initializer = 'zeros', name = 'Pixel_shuffle_scale3')(x)
...
def train():
    ...
    edsr_model.load_weights('checkpoint/EDSR_checkpoint_scale3.h5')
    
    '''
    data, label = ps.read_training_data('training_sample/train_DIV2Kaug_scale3_merge_2.h5')
    ...
    checkpoint_filepath = 'checkpoint/EDSR_checkpoint_scale3_merge.h5'
